...
loop do
  begin
    ...
    unless page.empty?
      page.each { |tweet|
        begin
          ...
          puts "Had a problem posting tweet #{tweet.id} to Evernote."
          $! = TRUE
        end
    ...
    minutes_unitl_rate_limit_is_reset = ( base.rate_limit_status.reset_time_in_seconds - Time.now.to_i ) / 60.0
    puts "Sorry, we've exceeded the Twitter-imposed rate limit for accessing their service.  We'll have to wait #{minutes_unitl_rate_limit_is_reset} minutes before this account can access Twitter again."
    exit
  end
end
